...
export async function POST(
  ...
  try {
    ...
    // Approve the member
    mess.members[targetMemberIndex].isActive = true
    await mess.save()
    ...
    await Notification.create({
      ...
      message: `Your request to join ${mess.name} has been approved. Welcome to the mess!`,
      relatedData: { messId: mess._id, messName: mess.name },
      priority: 'high',
    })

    return NextResponse.json({ 
      message: `${targetUser?.name || targetUser?.email || 'User'} has been approved and added to the mess` 
    })
  } catch (error) {
